package com.gentle.store.shopping.cart.config;

import com.gentle.store.shopping.cart.security.Role;
import com.gentle.store.shopping.cart.util.Constants;
    ...
    default SecurityFilterChain securityFilterChain(final HttpSecurity http) throws Exception {
        return http
            .authorizeHttpRequests(authorize -> {
                final var restPathKundeId = STR."\{Constants.SHOPPING_CART_PATH}/*";
                authorize
                        .requestMatchers(GET, Constants.SHOPPING_CART_PATH).hasRole(Role.ADMIN.name())
                        .requestMatchers(GET, restPathKundeId).hasAnyRole(Role.ADMIN.name(), Role.CUSTOMER.name())
                        .requestMatchers(POST, restPathKundeId).permitAll()
