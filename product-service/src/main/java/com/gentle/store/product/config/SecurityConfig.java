package com.gentle.store.product.config;

import com.gentle.store.product.security.Role;
import com.gentle.store.product.util.Constants;
    ...
    default SecurityFilterChain securityFilterChain(final HttpSecurity http) throws Exception {
        return http
            .authorizeHttpRequests(authorize -> {
                final var restPathKundeId = STR."\{Constants.PRODUCT_PATH}/*";
                authorize
                        .requestMatchers(GET, Constants.PRODUCT_PATH).hasRole(Role.ADMIN.name())
                        .requestMatchers(GET, restPathKundeId).hasRole(Role.ADMIN.name())
                        .requestMatchers(POST, restPathKundeId).hasRole(Role.ADMIN.name())
