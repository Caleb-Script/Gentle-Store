...
import static com.gentle.store.inventory.util.Constants.*;
...
public class UriHelper {
    ...
    public URI getBaseUri(final HttpServletRequest request) {
        ...
        final var baseUri =
            STR."\{uriComponents.getScheme()}://\{uriComponents.getHost()}:\{uriComponents.getPort()}\{INVENTORY_PATH}";
    ...
    private URI getBaseUriForwarded(final HttpServletRequest request, final String forwardedHost) {
        ...
        if (forwardedPrefix == null) {
            ...
            forwardedPrefix = Inventory_PREFIX;
        }
        final var baseUri = STR."\{forwardedProto}://\{forwardedHost}\{forwardedPrefix}\{INVENTORY_PATH}";
